...
def closest_boundary(t_sec: float, payload: dict[str, Any], max_delta_sec: float = 1.0) -> float | None:
    ...
    for seg in segments:
        if not isinstance(seg, dict):
            continue
        try:
            boundaries.append(float(seg["start"]))
            boundaries.append(float(seg["end"]))
        except (KeyError, TypeError, ValueError):
            continue
    if not boundaries:
        return None
    nearest = min(boundaries, key=lambda b: abs(b - t_sec))
    if abs(nearest - t_sec) <= max_delta_sec:
        return nearest
    return None


def segment_contains_time(payload: dict[str, Any], t_sec: float) -> bool:
...
def validate_and_repair_selection(
    ...
    *,
    ...
    for idx, seg in enumerate(segments, start=1):
        ...
        if video_duration_sec is not None and (start_sec < 0 or end_sec > video_duration_sec):
        ...
        repaired_start = closest_boundary(start_sec, transcript_payload)
        repaired_end = closest_boundary(end_sec, transcript_payload)
        if repaired_start is None or repaired_end is None:
            if not (segment_contains_time(transcript_payload, start_sec) and segment_contains_time(transcript_payload, max(start_sec, end_sec - 0.001))):
                issues.append(categorize_error("selection_validation", "timestamp_error", "Timestamps not grounded in transcript segments", {"index": idx}))
                continue
            repaired_start = start_sec
            repaired_end = end_sec

        if repaired_start >= repaired_end:
            issues.append(categorize_error("selection_validation", "timestamp_error", "Repaired timestamps invalid", {"index": idx, "start_sec": repaired_start, "end_sec": repaired_end}))
            continue

        repaired = dict(seg)
        repaired["start"] = _format_hhmmss(repaired_start)
        repaired["end"] = _format_hhmmss(repaired_end)
        repaired["duration_sec"] = round(repaired_end - repaired_start, 3)
