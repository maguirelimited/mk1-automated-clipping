...
from mk04_utils import (
    ...
    resolve_whisper_model_for_transcription,
)
...
def run_whisper_cli_transcription(input_video: str) -> str:
    ...
    write_debug_agent(
        "transcribe-video",
        "H9-transcribe-entry",
        "transcribe_video.py:run_whisper_cli_transcription",
        "entered Whisper CLI transcription runner",
...
def run_transcription(input_video: str) -> str:
    ...
    if engine == "whisperx":
        from transcribe_whisperx import run_whisperx_transcription

        return run_whisperx_transcription(input_video)
    return run_whisper_cli_transcription(input_video)
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        print(
            f"[SUCCESS] Transcription complete ({engine}) for {args[0]}",
            file=sys.stderr,
        )
        ...
        write_debug_agent(
            ...
            {"error": str(e), "engine": engine},
        )
        print(f"[ERROR] Transcription failed ({engine}): {e}", file=sys.stderr)
