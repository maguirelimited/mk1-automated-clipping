...
def _flat_extract_playlist_items(url: str, playlist_items: str) -> dict[str, Any]:
    ...
    detail(log, "Checking source with yt-dlp: url=%s playlist_items=%s", url, playlist_items)
    ...
    if hydrate_missing_duration:
        ...
        for cand in results:
            if not bool(cand.extra.get("hydrate_missing_duration", True)):
                ...
                detail(log, "Hydrating candidate metadata: url=%s", cand.url)
                hydrated.append(_hydrate_metadata(cand))
            else:
                hydrated.append(cand)
        results = hydrated

    detail(log, "Candidate source check complete: candidates=%s", len(results))
        ...
        for index in range(1, local_max + 1):
            ...
            for entry in entries:
                ...
                if seen is not None and seen.is_seen(video_id=cand.video_id, url=cand.url):
                    detail(log, "Skipping seen candidate before hydration: url=%s", cand.url)
                    continue
